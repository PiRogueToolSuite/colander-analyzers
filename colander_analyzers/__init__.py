...
# Copyright (C) 2022 Esther Onfroy
#
...
from colander_analyzers.modules.model import ColanderJsonEncoder
...
log = logging.getLogger('colander-analyzers')
...
def load_package_helpers():
    if not HAS_PACKAGE_HELPERS:
        log.info('Unable to load Colander analyzers from package.')
        sys.exit()
    mhandlers = {}
    helpers = []
    for path, helper in sys.modules.items():
        if not path.startswith('colander_analyzers.helpers.'):
            continue
        helpername = path.replace('colander_analyzers.helpers.', '')
    ...
    for root, dirnames, filenames in os.walk(mod_dir):
        ...
        for filename in fnmatch.filter(filenames, '*.py'):
            ...
            try:
                ...
                log.warning('Colander analyzers {0} failed due to {1}'.format(modulename, e))
                continue
            modules.append(modulename)
            log.info('Colander analyzers {0} imported'.format(modulename))
...
def load_package_modules():
    ...
    for path, module in sys.modules.items():
        r = re.findall(r"colander_analyzers[.]modules[.](\w+)[.]([^_]\w+)", path)
        if r and len(r[0]) == 2:
...
class ListModules(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        for module in loaded_modules:
            ...
            x['attributes'] = mhandlers[module].introspection()
...
class QueryModule(tornado.web.RequestHandler):
    ...
    def run_request(self, module, jsonpayload):
        log.debug('MISP QueryModule request {0}'.format(jsonpayload))  # ToDo remove debug
        response = mhandlers[module].handler(q=jsonpayload)
        return json.dumps(response, cls=ColanderJsonEncoder)
    ...
    def post(self):
        try:
            ...
            dict_payload['input']['request_id'] = str(uuid.uuid4())
            dict_payload['input']['requested_at'] = datetime.datetime.now().isoformat()
            if dict_payload.get('timeout'):
                timeout = datetime.timedelta(seconds=int(dict_payload.get('timeout')))
            else:
                timeout = datetime.timedelta(seconds=300)
            response = yield tornado.gen.with_timeout(timeout, self.run_request(dict_payload['module'], json.dumps(dict_payload)))
            self.write(response)
...
def _launch_from_current_dir():
    log.info('Launch Colander analyzers server from current directory.')
...
def main():
    ...
    if args.devel:
        ...
        if args.s:
            log.info('Launch Colander analyzers server from package.')
    ...
    log.info('Colander analyzers server started on {0} port {1}'.format(listen, port))
    if args.t:
        log.info('Colander analyzers started in test-mode, quitting immediately.')
        sys.exit()
    try:
